...
$arm64RuntimeBuildPath = Join-Path $root "scripts\Build-TypeDuckArm64Runtime.ps1"
$arm64ArtifactTestPath = Join-Path $root "scripts\Test-TypeDuckNativeArm64Artifacts.ps1"
...
$arm64RuntimeBuild = Read-RequiredText $failures $arm64RuntimeBuildPath "scripts/Build-TypeDuckArm64Runtime.ps1"
$arm64ArtifactTest = Read-RequiredText $failures $arm64ArtifactTestPath "scripts/Test-TypeDuckNativeArm64Artifacts.ps1"
...
foreach ($workflow in @(
  ...
  Assert-Text $failures $workflow.Text 'repository:\s*TypeDuck-HK/librime' "$($workflow.Name) must checkout the TypeDuck librime fork."
  Assert-Text $failures $workflow.Text 'path:\s*TypeDuck-librime\b' "$($workflow.Name) must checkout librime into TypeDuck-librime."
  Assert-Text $failures $workflow.Text '-Arm64RimeSourceRoot\s+"?\$env:GITHUB_WORKSPACE\\TypeDuck-librime' "$($workflow.Name) must pass the librime checkout to native ARM64 packaging."
  Assert-Text $failures $workflow.Text 'Test-TypeDuckNativeArm64Artifacts\.ps1' "$($workflow.Name) must verify staged PE architectures."
...
Assert-Text $failures $build '"--build",\s*\$Arm64BuildDir,\s*"--config",\s*\$Configuration\s*\)' "Build script must build the complete ARM64 solution."
...
Assert-Text $failures $install 'ARM64 TypeDuckLauncher\.exe' "Install staging must require a native ARM64 launcher."
Assert-Text $failures $install 'Arm64MoqiImeSource' "Install staging must consume a separate native ARM64 runtime."
Assert-Text $failures $arm64RuntimeBuild '\$env:GOARCH\s*=\s*"arm64"' "Native runtime build must cross-compile the Go server for ARM64."
Assert-Text $failures $arm64RuntimeBuild 'architecture=arm' "Native runtime build must compile Boost dependencies for ARM64."
Assert-Text $failures $arm64RuntimeBuild 'Assert-Arm64Pe' "Native runtime build must reject non-ARM64 server and rime binaries."
Assert-Text $failures $arm64ArtifactTest '0xAA64' "Artifact validation must require ARM64 PE machine values."
Assert-Text $failures $arm64ArtifactTest 'TypeDuckRuntime\\input_methods\\rime\\rime\.dll' "Artifact validation must inspect packaged rime.dll."
